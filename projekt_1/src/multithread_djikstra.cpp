...
MultithreadDijkstraResult multithread_bidirectional_dijkstra(
    ...
    MultithreadDijkstraResult result;
    ...
    auto forward_worker = [&]() {
        ...
        while (!stop.load(std::memory_order_relaxed)) {
            ...
            if (!pq_f.empty()) top_f.store(pq_f.top().dist, std::memory_order_relaxed);
    ...
    auto backward_worker = [&]() {
        ...
        while (!stop.load(std::memory_order_relaxed)) {
            ...
            if (!pq_b.empty()) top_b.store(pq_b.top().dist, std::memory_order_relaxed);
